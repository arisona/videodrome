  ...
  let html = markdown
    ...
    .replace(/((?:^- .+$\n?)+)/gm, (match: string) => {
      ...
      const listItems = items
        ...
        .join('');
      return `<ul style="margin: 12px 0 0 0; padding-left: 20px;">${listItems}</ul>\n`;
    })
    // Convert headings
    .replace(
      /^# (.+)\n/gm,
    ...
    // Convert standalone bold text (section headers) with margins
    .replace(
      /\n\*\*(.+?)\*\*\n/g,
      '\n<strong style="display: inline-block; margin-top: 12px; font-weight: 600;">$1</strong>\n',
    )
    // Convert remaining bold text (inline, no margins)
