...
function updateFileTitle(slot: SlotState, slotId: 'A' | 'B') {
  ...
  if (fileName) {
    ...
    fileNameElement.textContent = `${slotId}: New Patch${dirtyIndicator}`;
...
function updateParamSlots(compositeModeId: string) {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    const param = compositeFunc?.parameters[i - 1];
...
export async function loadPatchIntoSlot(patchPath: string, patchName: string, slotId: 'A' | 'B') {
  ...
  await slot.fileController.load(patchPath, patchName);
}
...
export function initPerformer() {
  ...
  const fileControllerA = new EditorFileController(editorA, {
  ...
  const fileControllerB = new EditorFileController(editorB, {
    onStatusUpdate: (msg, isError) => {
